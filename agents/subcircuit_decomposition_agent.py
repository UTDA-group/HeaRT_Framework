...
def parse_spice_netlist(netlist_str: str):
    ...
    # Extract signal ports from the .subckt header line
    SIGNAL_PORTS = set()
    for line in netlist_str:
        ...
        if stripped.startswith(".subckt"):
            ...
            all_ports = {t for t in header_tokens[2:] if "=" not in t}  # skip ".subckt", circuit name, and param=val tokens
            SIGNAL_PORTS = all_ports - {r.lower() for r in SUPPLY_RAILS}
            break
    ...
    for line in netlist_str:
        ...
        # If not found in tokens (non Mosfet most probably), use first letter of device name
        if dev_type is None:
            if dev_name.lower().startswith("c"):
                dev_type = "cap"
            elif dev_name.lower().startswith("r"):
                dev_type = "res"
        ...
        if "mos" in dev_type:
            ...
            nets = tokens[1:3]
            value = tokens[3]
            G.add_node(dev_name, bipartite="device", type="resistor", params=value)
        ...
        # Capacitor
            ...
            nets = tokens[1:3]
            value = tokens[3]
            G.add_node(dev_name, bipartite="device", type="capacitor", params=value)
...
class SplitterAgent(Agent):
    ...
    def generate_subcircuits(self, filepath):
        ...
        try:
            ...
            mixed_signal_flag = self.parse_response_mixed_signal_flag(response)

        except Exception as e:
            print("\n[PARSE ERROR] Failed to parse splitter response")
            print(f"Error type: {type(e).__name__}")
            print(f"Error msg : {e}")
            print(response)
            # Save response to debug path
            debug_path = os.path.join(filepath, "debug_splitter_response.txt")
        ...
        # Update self.subcircuits and put this into a global context memory to be accessed by all the agents
        # Put self.subcircuits in the global context for use by other agents

        # Update global context with parsed info for use by other agents
        self.global_context.set("top_level_port_names", top_level_port_names)
        self.global_context.set("mixed_signal_flag", mixed_signal_flag)
    ...
    def parse_response_mixed_signal_flag(self, LLM_response):
        data = self._safe_parse_json(LLM_response)
        if data:
            mixed_signal_flag = data.get("mixed_signal_flag", None)
            return mixed_signal_flag
        else:
            return None
...
def main():
    parser = argparse.ArgumentParser(description="Subcircuit decomposition splitter")
    parser.add_argument("--circuit", required=True, help="Circuit ID from benchmark_circuits.json (e.g. ckt1)")
    parser.add_argument("--no-plot", action="store_true", help="Skip matplotlib plots (for headless runs)")
    ...
    generated_response = circuit_splitter_agent.generate_subcircuits(
        os.path.join(parent_netlist_path, "Generated_Subcircuits")
    )
    circuit_splitter_agent.global_context.visualize()
    circuit_splitter_agent.global_context.save(
        os.path.join(
            parent_netlist_path, "Generated_Subcircuits/circuit_global_context_NEW.json"
        )
    )
    """
    output_dir = os.path.join(parent_netlist_path, "Generated_Subcircuits")
    os.makedirs(output_dir, exist_ok=True)

    circuit_splitter_agent.generate_subcircuits(output_dir)
    circuit_splitter_agent.global_context.save(
        os.path.join(output_dir, "circuit_global_context.json")
    )

    tokens = circuit_splitter_agent.last_usage.get("total_tokens", 0)
    print(f"Prompt tokens:     {circuit_splitter_agent.last_usage.get('prompt_tokens')}")
    print(f"Completion tokens: {circuit_splitter_agent.last_usage.get('completion_tokens')}")
